...
public class TankClient extends Frame implements ActionListener {
	...
	//String path=this.getClass().getResource("./Images/weekup.wav").getPath();
	AePlayWave music=new AePlayWave("src/Images/weekup.wav");//音乐
	MenuBar jmb = null;
	...
	public void framPaint(Graphics g) {
		...
		Color c = g.getColor();
		g.setColor(Color.yellow); // 设置字体显示属性
		Toolkit tk = Toolkit.getDefaultToolkit();
		Image image = tk.getImage(TankClient.class.getResource("Images/icon.jpg"));
	    this.setIconImage(image);
		Font f1 = g.getFont();
		g.setFont(new Font("TimesRoman", Font.BOLD, 20));
		g.drawString("      剩余敌方坦克:", 200, 70);
		g.setFont(new Font("TimesRoman", Font.ITALIC, 30));
		g.drawString("" + tanks.size(), 400, 70);// 剩余敌方坦克数
		g.setFont(new Font("TimesRoman", Font.BOLD, 20));
		g.drawString(" 剩余生命值:", 500, 70);
		g.setFont(new Font("TimesRoman", Font.ITALIC, 30));
		g.drawString("" + homeTank.getLife(), 650, 70);// 生命值
		g.setFont(f1);

		if (tanks.size() == 0 && home.isLive() && homeTank.isLive()) {
			Font f = g.getFont();
			g.setFont(new Font("TimesRoman", Font.BOLD, 60)); // 判断是否赢得比赛
			this.otherWall.clear();//清除其他剩余的墙
			g.drawString("好样的，你赢了！ ", 210, 300);
			g.setFont(f);
			//System.out.println("好样的，你赢了！ ");
		}

		if (homeTank.isLive() == false) {//自己的坦克被击毁
			Font f = g.getFont();
			g.setFont(new Font("TimesRoman", Font.BOLD, 40));
			this.tanks.clear();
			this.bullets.clear();
			g.drawString("很遗憾，你输了！ ", 210, 300);
			g.setFont(f);
		...
		for (int i = 0; i < metalWall.size(); i++) {//坦克撞到金属墙
			MetalWall w = metalWall.get(i);
			homeTank.collideWithWall(w);
			w.draw(g);
		}

		for (int i = 0; i < otherWall.size(); i++) {//坦克撞到普通墙
			CommonWall cw = otherWall.get(i);
			homeTank.collideWithWall(cw);
			cw.draw(g);
		}

		for (int i = 0; i < homeWall.size(); i++) { //玩家坦克撞到自己家
	...
	public TankClient() {
		// printable = false;
		// 创建菜单及菜单选项
		//AePlayWave music=new AePlayWave("src/Images/觉醒之心.wav");
		music.start();
		jmb = new MenuBar();
		jm1 = new Menu("游戏");
		jm2 = new Menu("暂停/继续");
		...
		jmi10 = new MenuItem("开启");
		jmi11 = new MenuItem("关闭");
		...
		jmi10.setActionCommand("beginmusic");
		jmi11.addActionListener(this);
		jmi11.setActionCommand("stopmusic");
		...
		for (int i = 0; i < tanknumber; i++) { // 初始化n辆坦克
			if (i < 8) // 设置坦克出现的位置
	...
	private class PaintThread implements Runnable {//Runnable接口方式实现多线程
		public void run() {
			// TODO Auto-generated method stub
			while (printable) {//通过是否重绘来达到线程暂停的效果
				repaint();
				try {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("NewGame")) {
			...
			Bullets.speedY = 18;
			tanknumber=25;// 坦克数量
			//attack=100;// 子弹攻击力降低难度
			music.stop();
			this.dispose();
			new TankClient();
		}
		else if(e.getActionCommand().equals("beginmusic")){
			music.setflag(true);
		}
		else if(e.getActionCommand().equals("stopmusic")){
			music.setflag(false);
		}
	}
	public static void main(String[] args){
